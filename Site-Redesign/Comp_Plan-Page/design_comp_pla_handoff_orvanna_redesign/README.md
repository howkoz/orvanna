# Handoff: Orvanna visual system, member office, operations console, catalog and compensation plan

## Overview

A redesign of orvanna.io covering six surfaces:
...
4. **Shop catalog** (`shop.html`) — thirteen identical cards become one promoted pack, a scannable agent list, and a sticky PV rail.
...
| `CORRECTIONS-01-catalog.md` | Punch list against the implemented catalog page: one real bug (the rail rendering once per group instead of once), then finishing items. |
| `CORRECTIONS-02-printable-plan.md` | How to wire the printable plan in: the filename, the `build_dist.py` registration, and the exact anchor markup for `comp-plan.html`. |
| `Orvanna Review.dc.html` | The full design canvas — turns 9 → 1, newest first. `9a` catalog, `8a` operations, `7a` member office, `6a` home page; `5a`–`5d` palette study; `4a`–`4e` motion; `2a`–`3d` logo directions; the `REVIEW` section for the written critique of the current site. |
