...
| 12 | [The subscription engine](12-SUBSCRIPTION-ENGINE.md) | What the subscription program does and how it retries: four frequencies, member-picked billing dates, decline-code-aware retry ladders, the five named production failures made structurally impossible, and the staff console that drives it all. **Engine schema applied to the cloud 2026-08-16 and deliberately inert there; 75 of 75 proofs pass on a full simulated year; the console is built and awaiting its deploy round.** Real stored cards, the daily scheduler, and member dunning notices are not built yet, and the document says so. | Writer, from the architect's spec and the builder's proof run |
