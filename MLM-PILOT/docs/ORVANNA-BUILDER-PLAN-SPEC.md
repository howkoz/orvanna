# Orvanna Builder Plan Specification (lab plan five: 'orvanna_builder'), version 1.2.2

**Correction release 2026-08-17, v1.2.2 (the publish gates' price, paid; the
exposure numbers in 12A(e) were understated and the understatement originates
HERE).** No mechanism, no parameter, no rate, and no gate changes. The publish
verifier recomputed section 12A(e)'s exposure figures directly from run 86's
per-source waterfall traces and found this specification's own framing wrong in
three ways, each of which made the plan look better than it is. Since 12A(e) is
the one paragraph whose declared job is to state the exposure WITHOUT softening
it, these are corrected here at the source rather than only in the presentation.

1. **Proration frequency was understated by half.** This spec says "120 of 1,001
   sources prorate, 12.0 percent" and "about one source in eight hits the cap".
   Measured: 120 is the count of sources whose LAYER 2 factor binds; 83 bind at
   layer 3; **154 bind on at least one layer**, of which 34 bind only at layer 3.
   And the 1,001 denominator includes **360 sources with zero volume that can
   never bind**. Against the 641 sources that have any pool at all, the true rate
   is **154 of 641 = 24.0 percent, roughly ONE IN FOUR.** Numerator and
   denominator were both wrong and both wrong in the same direction. The
   field-facing sentence this spec commits to is amended to one in four.
2. **The worst-member ratio used a denominator the ceiling cannot reach.** This
   spec says "the worst RATIO is 21 percent of that member's gross claims". That
   reproduces only when "gross claims" includes the spine, which by construction
   is never prorated. Measured on the claims the ceiling can actually reach, the
   worst ratio is **34.5 percent** (member 44: 19.24 shaved of 55.80 structure
   claims, against 20.96 percent of 91.80 including spine). And the true worst
   case is not a ratio: **five members lost 100 percent of their structure
   claims**, sums of 1.60 to 5.52 prorated entirely away. All three figures are
   now the record, and the 34.5 is the honest headline.
3. **Section 6A's "120 of 1,001 census sources bind"** carries the same defect and
   is corrected to the same numbers. The inversion's live-in-any-month conclusion
   is unaffected and if anything strengthens.

Also corrected in section 13 rule 3: the participation companion "24 members hold
that position" omitted its qualifying condition. Measured: **34 members personally
sponsored at least one Builder in March; 24 of those were also QUALIFIED that
month.** The banded premium table is right to use 24, because an unqualified member
cannot earn and does not belong in an earnings comparison, but the sentence
describing the number must say which population it is. The per-band sample sizes are
also added, because a two-decimal mean with no denominator is the easiest thing in
this document for a hostile reader to dismiss: **n = 8 with a sponsored Builder in
every band**, against 141, 186 and 97 without.

Two source-record notes, so neither is rediscovered. The red-team review prints
GW-000044's generation 1 line as "claim 33.00 paid 22.47"; the engine line is
**22.46** (claim 33.0000, basis 2,200.00, rate 0.0150). And the anti-gaming claim
that "every self-funding attack loses at least 80 percent" (section 11) is true
only for commission generated BY the self-funded volume itself: it does NOT bound
self-funded volume bought to unlock pay on other people's volume, which is what the
100.00 qualification gate does. Section 11 is amended below accordingly.
...
record this is not a corner (COUNT CORRECTED v1.2.2, 2026-08-17): 154 of the 641
census sources that carry any volume bind on at least one layer, 24.0 percent,
roughly one in four; the previously printed "120 of 1,001" counted only the layer 2
factor and divided by a population including 360 sources with no volume. The
inversion is live in any presentation month and must be named before a hostile
consultant names it.
...
this structural, SCOPED CORRECTLY v1.2.2, 2026-08-17: for commission generated BY the
self-funded volume itself, no plan funded under the cap can return more than 20 cents
of commission on a self-funded dollar, so a self-funding attack of that kind loses at
least 80 percent, and this one loses 93. The bound does NOT extend to self-funded
volume bought to UNLOCK pay on other people's volume, which is precisely what the SV
100.00 qualification gate does, and the live plan's approved Instant Payout terms
would return 42.00 on a self-funded first order of 150.00, which is 28 cents on the
dollar. The 20-cent figure bounds one channel; stated as a universal law it is false,
and it was stated as one here until today.
...
realized presentation numbers, seeded March at the calibrated rates of record
(CORRECTED v1.2.2, 2026-08-17, and the correction runs against this plan): **154 of
the 641 sources that carry any volume prorate on at least one layer, 24.0 percent,
roughly ONE IN FOUR.** Of those, 120 bind at layer 2 (down from 333 at the draft
rates), 83 bind at layer 3, and 34 bind only at layer 3. The figure this paragraph
printed until today, "120 of 1,001 sources prorate, 12.0 percent", was wrong twice
over and in the same direction: it counted only the layer 2 factor, and it divided
by a population containing 360 sources with zero volume that can never bind. The
...
what the cap leaves, and in a given month about one member-month in four with any
volume is reduced." (Sentence corrected v1.2.2 from "one source in eight".)
...
at least one shaved line**; the worst-shaved member loses 324.14. On the worst RATIO
(CORRECTED v1.2.2, 2026-08-17): 21 percent is the shave against that member's gross
claims INCLUDING the spine, and the spine can never be prorated, so that denominator
flatters the number. Against the claims the cap can actually reach the worst ratio is
**34.5 percent** (member 44: 19.24 shaved of 55.80 structure claims; the 21 figure is
19.24 of 91.80). And the true worst case is not a ratio at all: **five members lost
100 PERCENT of their structure claims**, sums of 1.60 to 5.52 prorated entirely away.
All three figures are the record, and any presentation carries the 34.5 and the five
zeroed members, not the 21 alone. The exhibit that states the problem in
...
3. **The development premium is THE pull number; "wide reach" is never claimed.**
   ...
   participation counts, stated exactly (CORRECTED v1.2.2, 2026-08-17): 34 members
   personally sponsored at least one Builder and 24 of those were ALSO QUALIFIED
   that month, 24 being the number the banded table uses because an unqualified
   member cannot earn; the per-band sample size is n = 8 with a sponsored Builder in
   every band, against 141, 186 and 97 without, and the two-decimal means are never
   printed without it; 44 earn any structure money;
