...
| ORVANNA_3DS_MODE | **ADDED 2026-08-19.** Not a secret: a SETTING, and the only row here that is safe to read aloud. Chooses what each payment asks of the bank. `three_ds` requests authentication deterministically, `no_three_ds` declines it, and unset or `auto` omits the field so HyperSwitch and the connector decide. Unset is the default and is today's behaviour. **Pin routing to a 3-D Secure capable connector before setting `three_ds`**: Authorize.net declares it cannot do 3-D Secure, and demanding it from a connector that has said no is the August 2026 fault this setting exists to stop repeating by hand. An unrecognised value falls back to `auto` and is reported as `three_ds_setting_ignored` on the create-payment response. | create-payment |
